...
function TopBar({ toggleSidebar }: Props) {
  ...
  return (
    <div className="bg-[#0a0a0a] border-b border-[#1e1e1e] py-3.5 px-5 flex items-center justify-between shadow-sm">
      <div className="flex items-center">
        <button 
          onClick={toggleSidebar}
          className="md:hidden text-white p-1.5 rounded-md hover:bg-[#1e1e1e] active:bg-[#252525] transition-colors"
        >
          <Bars3Icon className="h-5 w-5" />
        </button><div className="flex items-center ml-2 md:ml-0">
          <div className="h-9 w-9 bg-[#10a37f] rounded-md flex items-center justify-center text-white mr-3">
            <svg width="20" height="20" viewBox="0 0 41 41" fill="none" xmlns="http://www.w3.org/2000/svg" strokeWidth="1.5" className="h-6 w-6">
          ...
          <h1 className="font-bold text-white text-xl tracking-tight">ChatAI</h1>
        </div>
      </div>
        {session && (
        <div className="flex items-center space-x-3">
          <span className="hidden md:block text-xs text-gray-400 mr-1">
            <span className="inline-block h-2 w-2 bg-green-500 rounded-full mr-1.5 align-middle"></span>
            Online
          </span>
          ...
          <div className="relative" ref={dropdownRef}>
            <button 
              ...
              className="flex items-center focus:outline-none focus:ring-2 focus:ring-[#10a37f]/30 rounded-full"
            ...
            >
              <img
                ...
                alt="Profile"
                className={`h-9 w-9 rounded-full cursor-pointer border-2 transition-all duration-200 
                  ${dropdownOpen ? 'border-[#10a37f] shadow-md scale-105' : 'border-[#1e1e1e]'} 
