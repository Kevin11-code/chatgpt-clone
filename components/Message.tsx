...
function Message({ message }: Props) {
  ...
  const processMessageText = (text: string) => {
    ...
    return blocks.map((block, blockIndex) => {
            ...
            {lines.map((line, i) => {
              ...
              return (
                <li key={i} className="flex items-start">
                  ...
                  <span>{processFormattedText(listContent)}</span>
                </li>
              );
            })}
          </ul>
      ...
      if (block.trim().startsWith('```') && block.trim().endsWith('```')) {
        ...
        return (
          <pre key={blockIndex} className="bg-[#1e1e1e] p-3 rounded-md my-3 overflow-x-auto">
      ...
      return (
        <div key={blockIndex} className="my-2">
          {lines.map((line, i) => {
            ...
            return (
              <p key={i} className="my-1">{processFormattedText(line)}</p>
            );
          })}
        </div>
      );
  ...
  const processFormattedText = (text: string) => {
    ...
    // First handle code snippets with backticks
    const codeRegex = /`([^`]+)`/g;
      ...
      return (
        <>
          {textWithCodeProcessed.map((part, i) => {
            ...
            if (i % 2 !== 0) {
              return <code key={i} className="bg-[#1e1e1e] px-1 py-0.5 rounded text-xs">{part}</code>;
            }
  ...
  return (
    <div
      className={`py-6 text-white ${
        isChatAI ? "bg-[#000] text-[#fff]" : "bg-[#111]"
      }`}
    >
      <div className="flex max-w-3xl mx-auto px-4 md:px-8 items-start">
        <div className="flex-shrink-0 mr-4 mt-1">
          {isChatAI ? (
            <div className="h-8 w-8 bg-[#10a37f] rounded-sm flex items-center justify-center text-white">
            ...
            <img src={message.user.avatar} alt="avatar" className="h-8 w-8 rounded-sm" />
          )}
        </div>
        <div className="prose prose-invert flex-grow max-w-full">
